...
pub fn init_db() -> Result<(), String> {
    ...
    // Attempt to add new columns (for legacy database migration, ignore existing errors)
    ...
    // Create token_ip_bindings table
    conn.execute(
...
pub fn record_token_usage_and_ip(
    ...
    // 1. Update user_tokens main table
    tx.execute(
    ...
    // 2. Update or insert into token_ip_bindings table
    ...
    // 3. Insert into token_usage_logs table
...
// Check if token is valid (including expiration and IP limit checks)
...
pub fn validate_token(token_str: &str, ip: &str) -> Result<(bool, Option<String>), String> {
    ...
    if let Some(token) = token_opt {
        ...
        if token.max_ips > 0 {
            ...
            // Check if current IP is already bound
            ...
            if !is_bound {
                // If not bound, check if limit reached
        ...
        // 3. Check curfew time
        // Logic: Reject if current server time is between start and end
        // Format: HH:MM
        if let (Some(start_str), Some(end_str)) = (&token.curfew_start, &token.curfew_end) {
            if !start_str.is_empty() && !end_str.is_empty() {
                ...
                // Cross-midnight handling: start > end (e.g. 23:00 to 06:00)
...
// Get username associated with IP (for IP management page)
// Returns the username belonging to the token that last used this IP

...
pub fn save_log(log: &ProxyRequestLog) -> Result<(), String> {
    ...
    conn.execute(
        "INSERT INTO request_logs (id, timestamp, method, url, status, duration, model, error, request_body, response_body, input_tokens, output_tokens, account_email, mapped_model, protocol, client_ip, username, correlation_id, request_id)
    ...
    let logs_iter = stmt
        .query_map([limit, offset], |row| {
            Ok(ProxyRequestLog {
                ...
                correlation_id: row.get(17).unwrap_or(None),
                request_id: row.get(18).unwrap_or(None),
                error: row.get(7)?,
                request_body: None,
                response_body: None,

...
pub fn get_token_usage_by_ip(limit: usize, hours: i64) -> Result<Vec<IpTokenStats>, String> {
    ...
    // Database stores timestamp in milliseconds, but we were calculating 'since' in seconds
    // Convert 'hours' to milliseconds
    let since = chrono::Utc::now().timestamp_millis() - (hours * 3600 * 1000);

    // No longer retrieving username from the request_logs table because it might be empty
    // Fetch IP statistics first, then query each IP's username separately
    let mut stmt = conn.prepare(
        "SELECT
            client_ip,
    ...
    for row in rows {
        ...
        // Retrieve the username associated with the IP from user_token_db
        // This is more reliable than retrieving from request_logs because the token_ip_bindings table updates with every User Token usage

...
pub async fn monitor_middleware(
    ...
    //  Extract UserTokenIdentity from request extensions (injected by Auth middleware)
    // Must be extracted before processing request body, as this value needs to be preserved after into_parts()
    ...
    // user_token_identity has already been extracted from request extensions above
            ...
            Ok(bytes) => {
                if let Ok(s) = std::str::from_utf8(&bytes) {
                    if let Ok(json) = serde_json::from_str::<Value>(&s) {
                        // Supports OpenAI "usage" or Gemini "usageMetadata"

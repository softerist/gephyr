...
// Proxy configuration (used to build reqwest Client)
...
impl ProxyPoolManager {
    pub fn new(config: Arc<RwLock<ProxyPoolConfig>>) -> Self {
        // Load saved binding relationships from configuration
        let account_bindings = Arc::new(DashMap::new());

        // Read configuration using blocking method (since new is not async)
        // Note: use try_read here to avoid deadlocks
    ...
    //  Get the "effective" HttpClient for a specified account
    // Logic:
    // 1. Account explicit proxy binding takes priority (Account-Proxy Binding)
    // 2. If no binding and "auto global" is enabled, pick the first node in the pool
    // 3. If none of the above, check global upstream proxy (Upstream Proxy) [handled as fallback by caller]
    pub async fn get_effective_client(&self, account_id: Option<&str>, timeout_secs: u64) -> Client {
        ...
        let proxy_opt = if let Some(acc_id) = account_id {
            ...
            // General request without account_id: if proxy pool is enabled, pick a node from it as egress by default
            ...
            if config.enabled {
                ...
                if let Some(ref p) = res {
                    ...
                    //  Specifically record the case where no available proxy is in the pool
        ...
        if let Some(proxy_cfg) = proxy_opt {
            ...
            // Fallback to the single upstream proxy in app configuration
    ...
    pub async fn get_proxy_for_account(
        ...
        // 1. Prioritize account binding (Exclusive IP)
        ...
        // 2. Otherwise use strategy selection from the pool (Public Pool)
    ...
    async fn get_bound_proxy(
        ...
        if let Some(proxy_id) = self.account_bindings.get(account_id) {
            if let Some(entry) = config.proxies.iter().find(|p| p.id == *proxy_id.value()) {
                if entry.enabled {
                    // If auto failover is enabled and proxy is unhealthy, return None (will fallback to other strategies or fail)
    ...
    async fn select_proxy_from_pool(
        ...
        //  Exclusive isolation logic: exclude all bound proxies to protect the security of exclusive IP accounts
        ...
        let healthy_proxies: Vec<_> = config.proxies.iter()
            .filter(|p| {
                ...
                // If this proxy is "exclusively bound" to an account, it no longer participates in public round-robin
        ...
        if healthy_proxies.is_empty() {
             // If all proxies are bound, or the pool itself is empty, try to return proxies that are enabled in the pool and do not depend on binding
             // (This can be further adjusted based on business logic; currently maintaining strict isolation)
            return Ok(None);
        }
    ...
    fn select_weighted<'a>(&self, proxies: &[&'a ProxyEntry]) -> Option<&'a ProxyEntry> {
        // Simple implementation: weighted similar to priority; temporarily using Priority as a substitute
        self.select_by_priority(proxies)
    }

    // Build reqwest::Proxy configuration
    fn build_proxy_config(&self, entry: &ProxyEntry) -> Result<PoolProxyConfig, String> {
        //  Handle missing protocol scheme (e.g. "ip:port" -> "http://ip:port")
    ...
    pub async fn bind_account_to_proxy(
        ...
        // Check if proxy exists
        {
            ...
            // Check max accounts limit for the proxy
    ...
    pub async fn health_check(&self) -> Result<(), String> {
        // Since asynchronous concurrent checks are needed and the config cannot be locked for too long,
        // we first copy a list of proxies that need to be checked.
    ...
    // Check health status of a single proxy
    async fn check_proxy_health(&self, entry: &ProxyEntry) -> (bool, Option<u64>) {
        ...
        // Try to build Client; if it fails directly, consider it unhealthy

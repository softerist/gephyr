...
impl TokenManager {
    ...
    pub async fn load_accounts(&self) -> Result<usize, String> {
        ...
        for entry in entries {
            ...
            // Try loading account
            match crate::proxy::token::loader::load_single_account(&path, &self.health_scores).await
            {
                Ok(Some(token)) => {
    ...
    pub async fn mark_rate_limited_async(
        ...
        let account_id = crate::proxy::token::lookup::account_id_by_email(&self.tokens, email)
            .unwrap_or_else(|| email.to_string());

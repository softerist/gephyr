...
// Creates a transformation from Gemini SSE stream to Claude SSE stream
pub fn create_claude_sse_stream(
    mut gemini_stream: Pin<Box<dyn Stream<Item = Result<Bytes, reqwest::Error>> + Send>>,
    trace_id: String,
    email: String,
    session_id: Option<String>, // [NEW v3.3.17] Session ID for signature caching
    scaling_enabled: bool, //  Flag for context usage scaling
    context_limit: u32,
    estimated_prompt_tokens: Option<u32>, //  Estimated tokens for calibrator learning
    message_count: usize, //  Message count for rewind detection
    client_adapter: Option<std::sync::Arc<dyn ClientAdapter>>, //  Adapter reference
    ...
    Box::pin(stream! {
        ...
        state.message_count = message_count; //  Set message count
        state.scaling_enabled = scaling_enabled; // Set scaling enabled flag
        state.context_limit = context_limit;
        state.estimated_prompt_tokens = estimated_prompt_tokens; //  Pass estimated tokens
        state.set_client_adapter(client_adapter); //  Set adapter
        let mut buffer = BytesMut::new();

        loop {
            //  30s heartbeat: extended timeout to be compatible with high-latency models
        ...
        // Post-thinking interruption recovery
...
fn process_sse_line(line: &str, state: &mut StreamingState, trace_id: &str, email: &str) -> Option<Vec<Bytes>> {
    ...
    // Unwrap response field (if it exists)
...
mod tests {
    ...
    async fn test_thinking_only_interruption_recovery() {
        ...
        let mock_stream = async_stream::stream! {
            ...
            // Then suddenly ends (No Text, No Usage, directly None)
        };
        ...
        // Must contain recovered system prompt
        assert!(output.contains("Recovered by Antigravity"));
        
        // Must contain simulated Usage
